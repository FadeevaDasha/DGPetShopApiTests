...
class TestPet:
    ...
    @allure.title('Попытка удалить несуществуещего питомца')
    ...
    def test_updating_information_pet(self, create_pet):
        with allure.step('Получение ID созданного питомца'):
            pet_id = create_pet['id']
        ...
        with allure.step('Отправление PUT-запроса с подготовленными данными'):
            response = requests.put(url=f'{BASE_URL}/pet', json=payload)
            response_json = response.json()
